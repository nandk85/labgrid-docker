...
from ..protocol import InfoProtocol, CommandProtocol, PowerProtocol
from .common import Driver


@target_factory.reg_driver
@attr.s(cmp=False)
class GatewayDriver(Driver, InfoProtocol):
    """
    GatewayDriver implementing the InfoProtocol on
    top of CommandProtocol drivers
    """
    ...
    @Driver.check_active
    @step(args=['interface'])
    def get_ip(self, interface="erouter0", ipmode="ipv4"):
        """Returns the IP of the supplied interface"""
        try:
            if ipmode == 'ipv4':
                ip_string = self.command.run_check("ip -o -4 addr show")
            elif ipmode == 'ipv6':
                ip_string = self.command.run_check("ip -o -6 addr show")
        ...
        if ipmode == 'ipv4':
            ...
            regex = re.compile(
                r"""\d+:       # Match the leading number
                \s+(?P<if>\w+) # Match whitespace and interfacename
                \s+inet6*\s+(?P<ip>[\w:]+) # Match IP Adress
                /(?P<prefix>\d+) # Match prefix
                .*global # Match global scope, not host scope""", re.X
            )
    ...
    def reset(self):
        try:
            ...
            self.logger.debug('Unavailable to reboot device')
            return None
        return True
